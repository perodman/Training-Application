...
function renderCalendar() {
    ...
    // LÅST FORMAT: "Maj 2026"
    const monthNameRaw = currentViewDate.toLocaleString('sv-SE', { month: 'long' });
    label.textContent = monthNameRaw.charAt(0).toUpperCase() + monthNameRaw.slice(1) + " " + year;
    ...
    if (completed.length > 0) {
        completed.forEach((w, idx) => {
            html += `<div class="card glass" style="text-align:left;">
                <div style="display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <button onclick="deleteLoggedWorkout('${dateStr}', ${idx})" style="background:none; border:none; color:var(--danger); font-size:20px; cursor:pointer;">✖</button>
                </div>
            </div>`;
        });
    } else if (isOngoing) {
        html += `<button class="mode-btn orange" onclick="closeModal(); renderActiveWorkout()">Fortsätt pass</button>`;
        ...
        programData.routine.forEach(p => {
            html += `<div style="display:flex; gap:5px; margin-bottom:5px;">
                ...
                <button onclick="previewPass('${p.id}')" style="background:var(--card); border:1px solid var(--glass-border); border-radius:12px; width:45px; color:var(--primary); cursor:pointer;">ℹ️</button>
            </div>`;
        });
...
// --- ACTIVE WORKOUT (MED PILAR & REAL-TIME SAVE) ---
        ...
        card.innerHTML = `
            <div style="display:flex; align-items:center; margin-bottom:12px;">
                <div class="move-ctrls">
                    <button class="move-btn-small" onclick="moveActiveExercise(${i}, -1)">▲</button>
                    <button class="move-btn-small" onclick="moveActiveExercise(${i}, 1)">▼</button>
                </div>
                <strong style="flex-grow:1;">${ex.name}</strong>
            </div>
            <div style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap:10px;">
                <input type="number" id="w-${i}" class="log-input" placeholder="KG" value="${val.weight}" oninput="updateDraftData(${i})">
                <input type="number" id="r-${i}" class="log-input" placeholder="Reps" value="${val.reps}" oninput="updateDraftData(${i})">
                <input type="number" id="s-${i}" class="log-input" placeholder="Set" value="${val.sets}" oninput="updateDraftData(${i})">
            </div>`;
...
function moveActiveExercise(index, direction) {
    ...
    if (newIndex >= 0 && newIndex < activeDraft.workout.exercises.length) {
        const tempEx = activeDraft.workout.exercises[index];
        activeDraft.workout.exercises[index] = activeDraft.workout.exercises[newIndex];
        activeDraft.workout.exercises[newIndex] = tempEx;
        const tempData = activeDraft.data[index];
        activeDraft.data[index] = activeDraft.data[newIndex];
        activeDraft.data[newIndex] = tempData;
        saveAll();
        renderActiveWorkout();
    }
}
...
function cancelOngoing() { if(confirm("Radera pågående?")) { activeDraft = null; saveAll(); closeModal(); renderCalendar(); } }
...
function openEditProgramModal(idx) {
    ...
    body.innerHTML = `
        ...
        <div id="edit-pass-exercises">
            ${pass.exercises.map((ex, i) => `<div style="display:flex; justify-content:space-between; margin-bottom:8px;">
                <span>${ex.name}</span>
...
function addExToPass(pIdx, exId) { if(!exId) return; const ex = masterExercises.find(e => e.id == exId); programData.routine[pIdx].exercises.push({...ex}); openEditProgramModal(pIdx); }
...
function filterExercises(category) {
    ...
    .forEach(ex => {
        results.innerHTML += `<div class="card glass" style="display:flex; justify-content:space-between; align-items:center;">
...
function deleteMasterExercise(id) { if(confirm("Radera?")) { masterExercises = masterExercises.filter(e => e.id != id); saveAll(); filterExercises(currentExerciseCategory); } }
