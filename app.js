...
function filterExercises(category) {
    ...
    if(filtered.length === 0) {
        results.innerHTML = `<p style="text-align:center; color:var(--text-light); margin-top:20px;">Inga övningar i denna kategori än.</p>`;
    }
...
function editLoggedWorkout(date, idx) {
    const filtered = workoutHistory.filter(w => w.date === date);
    const item = filtered[idx];
    const workoutObj = { id: "edit-" + Date.now(), name: item.programName, exercises: item.exercises.map(ex => ({ name: ex.name, target: "" })) };
    const dataObj = item.exercises.map(ex => ({ weight: ex.weight, reps: ex.reps, sets: ex.sets }));
    
    // Radera den gamla loggen först så den ersätts
    workoutHistory = workoutHistory.filter(w => w !== item);
    saveAll();
    closeModal();
