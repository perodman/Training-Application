...
function renderActiveWorkout() {
    // --- BÖRJADE LÄGGA TILL ANTI-HOPP HÄR ---
    ...
    activeDraft.workout.exercises.forEach((ex, i) => {
        ...
        // --- ÄNDRING: Lade till VILA-rubrik ---
        let setsHtml = `<div style="margin-top:10px;">
            <div style="display:grid; grid-template-columns: 40px 1fr 1fr 40px 30px; gap:8px; margin-bottom:5px; align-items:center;">
        ...
        exerciseData.sets_data.forEach((set, sIdx) => {
            ...
            // --- ÄNDRING: Lade till timer-knapp ---
            setsHtml += `
            <div style="display:grid; grid-template-columns: 40px 1fr 1fr 40px 30px; gap:8px; margin-bottom:8px; align-items:center;">
                ...
                <button onclick="event.stopPropagation(); startRestTimer()" style="background:rgba(255,255,255,0.05); border:none; border-radius:4px; color:var(--text-light); font-size:12px; height:36px;">⏱️</button>
        ...
        <div onclick="toggleExercise(${i})" style="padding: 12px 15px; display: flex; align-items: center; cursor: pointer; background: ${isOpen ? 'rgba(250, 204, 21, 0.05)' : 'transparent'}">
            ...
            <div style="display: flex; flex-direction: column; min-width:0; flex-grow:1;">
                ...
                <small style="color: ${isDone ? '#22c55e' : 'var(--primary)'}; font-size: 10px;">${isDone ? 'KLAR ✅' : `${completedSets}/${totalSets} set`}</small>
            </div>
